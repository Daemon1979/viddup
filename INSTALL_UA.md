...
Основний процес, hashing worker-и та дочірні FFmpeg-процеси за замовчуванням
працюють із nice level `10`. Змінити його можна через `--nice LEVEL` або
`nice = LEVEL` у секції `[common]`.

Стандартний метод `legacy-center` сумісний із базами оригінальної утиліти. Щоб
рахувати яскравість усього кадру, під час створення нової бази вибери
`full-frame`:
...
Обраний метод записується в SQLite-базу. Наступні імпорти та пошуки читають
його автоматично, тому повторювати `--hash-method` не потрібно. Перемкнути
метод для вже заповненої бази не можна, оскільки несумісні хеші не мають
змішуватися.
...
Кожен результат містить позицію знайденого збігу та метадані для порівняння
копій:
...
Повна тривалість береться з бази. Кодек і роздільна здатність паралельно
визначаються через `ffprobe` лише для файлів, що потрапили у фінальні групи,
після чого кешуються в SQLite. Для старих баз таблиця кешу створюється
автоматично. Наступний пошук використовує кеш, а `--refresh` інвалідує запис
відповідного файла.
...
Метод для нової бази також можна задати в конфігу:
