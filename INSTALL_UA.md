...
Fingerprint описує часові інтервали між максимумами яскравості, тому іноді
різні відео можуть випадково дати схожий рисунок. Якщо хибних збігів багато,
спочатку спробуй довший fingerprint:

```sh
dupfind --db videos.db --search --indexlength 12
```

Якщо пошук усе ще надто поблажливий, другим кроком зменш radius:

```sh
dupfind --db videos.db --search --indexlength 12 --radius 2
```

Ці параметри підвищують точність ціною чутливості. Більший `--indexlength`
може приховати короткі реальні збіги, а менший `--radius` - дублікати після
зміни FPS, монтажу або кодування. Якщо важливіше знайти максимум справжніх
дублікатів і вручну відкинути рідкісні помилки, залишай стандартні значення.
