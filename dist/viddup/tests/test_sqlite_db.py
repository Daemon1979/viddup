...
def test_get_and_delete_files_under_path(tmp_path: Path):
    ...
    remove_one = str(tmp_path / "Games" / "one.mp4")
    remove_two = str(tmp_path / "Games" / "nested" / "two.mp4")
    not_remove = str(tmp_path / "Games_old" / "three.mp4")

    for path in (keep, remove_one, remove_two, not_remove):
        db.insert_file(path, fps=25.0, duration=10.0)
    db.commit()

    matches = db.get_file_infos_under_path(str(tmp_path / "Games"))

    assert sorted(item.name for item in matches) == sorted([remove_one, remove_two])

    deleted = db.del_files_under_path(str(tmp_path / "Games"))
